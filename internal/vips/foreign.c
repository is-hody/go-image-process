...
int set_pngsave_options(VipsOperation *operation, SaveParams *params) {
  ...
  if (!ret && params->quality) {
    ...
    if (!ret && params->effort) {
      ret = vips_object_set(VIPS_OBJECT(operation), "effort", params->effort, NULL);
    }
...
int set_png_magicksave_options(VipsOperation *operation, SaveParams *params) {
  int ret = vips_object_set(VIPS_OBJECT(operation), "format", "PNG", NULL);
  if (!ret && params->quality) {
    ret = vips_object_set(VIPS_OBJECT(operation), "quality", params->quality,
                          NULL);
  }
  if (!ret && params->pngBitdepth) {
      ret = vips_object_set(VIPS_OBJECT(operation), "bitdepth", params->pngBitdepth,
                            NULL);
    }
  return ret;
}
...
int save_to_buffer(SaveParams *params) {
  switch (params->outputFormat) {
    ...
    case PNG:
//      return save_buffer("magicksave_buffer", params, set_png_magicksave_options);
...
static SaveParams defaultSaveParams = {
    ...
    .pngBitdepth = 0,
    .pngDither = 0,
    .pngFilter = VIPS_FOREIGN_PNG_FILTER_AVG,
